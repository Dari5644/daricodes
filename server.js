// server.js (نسخة تُصلّح خطأ "no such table")
...
// نستخدم data.db بجانب server.js (أو ارفع DATA_DIR لمسار ثابت)
const DB_PATH = process.env.DATA_DIR
  ? path.join(process.env.DATA_DIR, "data.db")
  : path.join(__dirname, "data.db");

// لو تبغى إيقاف التشغيل إذا القاعدة غير موجودة، علّق السطر التالي
// لكن بما إننا بنعمل إنشاء تلقائي، خليه يكمل حتى لو فاضية.
if (!fs.existsSync(DB_PATH)) {
  console.warn("⚠️ data.db غير موجود — سيتم إنشاؤه تلقائيًا:", DB_PATH);
}
...
if (!hasOwner) {
  ...
  console.log("✅ مالك تم إنشاؤه:", OWNER_EMAIL);
}

const snipCount = db.prepare("SELECT COUNT(*) c FROM snips").get().c;
if (!snipCount) {
  ...
  console.log("✅ تم إدخال أكواد أولية");
}

const app = express();
if (FRONTEND_ORIGIN) app.use(cors({ origin: FRONTEND_ORIGIN, credentials: true })); else app.use(cors());
...
app.get("/admin.html",(req,res,next)=>{
  if(!req.session.user || !req.session.user.is_owner){
    return res.status(403).send("<h2 style='text-align:center;margin-top:100px;'>🚫 غير مصرح لك بدخول لوحة المالك</h2>");
  }
  next();
});
