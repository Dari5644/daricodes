...
// أعط كل زائر معرف جلسة ثابت (تثبيت المحادثة)
...
function readDB(){ if(!fs.existsSync(DB)) fs.writeFileSync(DB, JSON.stringify({users:[],snips:[],messages:[]},null,2)); return JSON.parse(fs.readFileSync(DB,"utf-8")); }
function writeDB(d){ fs.writeFileSync(DB, JSON.stringify(d,null,2)); }

// seed أول مرة
(function seed(){
  const db = readDB();
  if (!db.users.find(u=>u.email===OWNER_EMAIL)) db.users.push({id:uuid(),email:OWNER_EMAIL,name:"ضاري",password:OWNER_PASSWORD,is_owner:true});
  if (!db.snips.length){
    const now=Date.now(), owner=OWNER_EMAIL, oname="ضاري";
    const add=(t,l,d,c)=>db.snips.push({id:uuid(),title:t,lang:l,description:d,code:c,owner_email:owner,owner_name:oname,created_at:now});
    // Python
    add("حساب العاملي","python","n!","def fact(n):\n r=1\n for i in range(2,n+1): r*=i\n return r\nprint(fact(5))");
    add("سلسلة فيبوناتشي","python","أول 10","a,b=0,1\nfor _ in range(10):\n print(a)\n a,b=b,a+b");
    add("Palindrome","python","تناظر","s='رادار'\nprint(s==s[::-1])");
    ...
    add("نسخ للحافظة","javascript","Clipboard","async function copy(t){await navigator.clipboard.writeText(t);console.log('copied')};copy('مرحبا');");
    ...
    // SQL
    add("إنشاء users","sql","Create","CREATE TABLE users(id INTEGER PRIMARY KEY AUTOINCREMENT,email TEXT);");
...
app.post("/api/auth/login",(req,res)=>{
  ...
  const db=readDB(); const u=db.users.find(x=>x.email===email&&x.password===password);
...
app.get("/api/snips",(req,res)=>{
  const {q="",lang="all"}=req.query; const k=q.trim().toLowerCase();
  const db=readDB();
  const list=db.snips.filter(s=>(lang==="all"||s.lang===lang)&&(!k||(s.title+s.description+s.code).toLowerCase().includes(k)))
    .sort((a,b)=>b.created_at-a.created_at);
  res.json(list);
});
app.post("/api/snips", mustLogin,(req,res)=>{
  const {title,lang,description,code}=req.body||{};
  const db=readDB(); const me=req.session.user; const id=uuid();
  db.snips.push({id,title,lang,description,code,owner_email:me.email,owner_name:me.name||me.email,created_at:Date.now()});
  writeDB(db); res.json({ok:true,id});
});
app.delete("/api/snips/:id", mustOwner,(req,res)=>{
  const db=readDB(); const before=db.snips.length;
  db.snips=db.snips.filter(s=>s.id!==req.params.id); writeDB(db);
  res.json({ok:true,deleted:before-db.snips.length});
});

// --- MESSAGES (خدمة العملاء) — خيط زائر + رد خاص من المالك
// للمالك: عرض كل الرسائل
app.get("/api/messages", mustOwner,(req,res)=>{
  const db=readDB(); res.json(db.messages.sort((a,b)=>a.created_at-b.created_at));
});
// للزائر: خيطه الخاص فقط
app.get("/api/messages/thread",(req,res)=>{
  const db=readDB(); const vid=req.session.visitor_id; const me=req.session.user;
  const t=db.messages.filter(m=> me ? (m.user_email===me.email || m.visitor_id===vid) : (m.visitor_id===vid))
...
app.post("/api/messages",(req,res)=>{
  ...
  const db=readDB(); const me=req.session.user;
  const msg={
    id:uuid(), visitor_id:req.session.visitor_id,
    user_email: me?me.email:"", user_name: me?(me.name||me.email):"زائر",
    content, reply:"", created_at:Date.now()
  };
  db.messages.push(msg); writeDB(db); res.json({ok:true,id:msg.id});
});
// رد المالك على رسالة واحدة (يظهر لصاحبها فقط)
app.post("/api/messages/:id/reply", mustOwner,(req,res)=>{
  const { reply="" } = req.body||{}; const db=readDB();
  const m=db.messages.find(x=>x.id===req.params.id); if(!m) return res.status(404).json({ok:false});
  m.reply=reply; writeDB(db); res.json({ok:true});
});
